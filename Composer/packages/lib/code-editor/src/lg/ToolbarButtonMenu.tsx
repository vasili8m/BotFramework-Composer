...
export const ToolbarButtonMenu = React.memo((props: ToolbarButtonMenuProps) => {
  ...
  const { onRenderMenuList, query, setQuery } = useSearchableMenuListCallback(uiStrings.searchPlaceholder, () => (
  ...
  React.useEffect(() => {
    if (query) {
      ...
      const predictable = getFilterPredictable(payload.kind, query);
